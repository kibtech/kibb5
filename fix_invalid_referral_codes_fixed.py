#!/usr/bin/env python3
"""
Fix Invalid Referral Codes - Fixed Version
This script addresses users with referral codes that don't exist in the system.
It properly handles unique constraints and duplicate referral codes.
"""
        ...
        for user in users_with_issues:
            ...
            if referrer and referrer.id != user.id:
                ...
                if potential_referrer:
                    # Set the referral relationship but DON'T change the referral code
                    # This avoids duplicate referral code issues
                    ...
                    logger.info(f"✅ Generated new referral code: {new_referral_code}")
                else:
                    # Clean up the invalid referral code and generate a new one
                    new_referral_code = generate_unique_referral_code()
                    user.referral_code = new_referral_code
                    cleaned_count += 1
...
if __name__ == "__main__":
    try:
        print("🔧 FIXING INVALID REFERRAL CODES - FIXED VERSION")
        print("=" * 60)
